...
public interface BlogMapper {
    @Select("SELECT a.id, a.user_id, a.title, a.content, a.comment_count, a.like_count, a.created_at, a.deleted_at, a.tag_id, a.is_hot, a.images, b.name, b.nick_name, b.company, b.hospital, b.user_pic, b.authen_status FROM blog a, user_info b where a.user_id = b.id order by created_at DESC")
...
//    @Select("SELECT * from blog, user_info where blog.user_id = user_info.id and blog.is_hot = 2 or blog.user_id = user_info.id and blog.is_hot = 1 order by blog.is_hot desc")
    ...
    //大咖说微博
    @Select("SELECT * from blog, user_info where blog.user_id = user_info.id and user_info.authen_status = 'A' or blog.user_id = user_info.id and user_info.authen_status = 'C' order by created_at DESC")
