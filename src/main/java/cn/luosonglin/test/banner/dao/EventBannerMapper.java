...
//    @Select("select id, title, banner, start_date, end_date from event where end_date >= CURRENT_TIMESTAMP and banner !='' and id >= 207 and id <=211 order by id asc")
    @Select("select a.id, a.title, a.start_date, a.end_date, b.banner from event a, phoneweb_set b where a.id = b.event_id and a.id = 180 or a.id = b.event_id and a.id = 210 order by a.id asc")
